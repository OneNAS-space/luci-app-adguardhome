...
Check_Task(){
        ...
        *)
                # 【对齐运行锁】如果锁存在或有其它实例运行
                if [ -f /var/run/update_core_running ] || [ "${running_tasks}" -gt 2 ]; then
...
# 【核心修正】通过基本自检后立即落锁，让前端第一时间稳稳捕捉到运行状态，绝不空跑
touch /var/run/update_core_running 2>/dev/null

# 针对突发中断信号（如进程被杀）的兜底 Trap 逻辑，避免留存死锁
trap 'touch /var/run/update_core_error 2>/dev/null; cleanup_lockfile; exit 1' SIGINT SIGTERM
